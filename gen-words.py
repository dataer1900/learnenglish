"""Regenerate word audio using edge-tts for cleaner pronunciation."""
...
async def main():
    ...
    for i, w in enumerate(words):
        ...
        n = i + 1
        sys.stdout.write(f'\r[{n}/{len(words)}] {word}          ')
        ...
        try:
            tasks.append(_dl(word, VOICES['uk'], uk_path))
            tasks.append(_dl(word, VOICES['us'], us_path))
    ...
    print(f'\n\nDone. OK: {ok}  Fail: {fail}  Total: {len(words)}')
